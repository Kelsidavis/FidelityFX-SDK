...
#define FSR4_ASSERT(_cond) do { if (!(_cond)) { fprintf(stderr, "FSR4 ASSERT: %s\n", #_cond); } } while(0)
...
static bool EnableAMDExtensions(void* device)
{
    // TODO: query VK_KHR_cooperative_matrix for WMMA support
    (void)device;
    return false; // Use FP8 path by default
}

bool ffxProvider_FSR4::IsSupported(void* device) const
{
    (void)device;
    return true; // FP8 path works on all GPUs with compute shader support
}
...
static ffxReturnCode_t CreateModel(InternalFsr4Context* internal_context, fsr4_shaders::Preset modelPreset)
{
    ...
    {
        swprintf(pipelineDescription.name, 64, L"FFX_FSR4_MODEL_PASS_%u", passIndex);
    ...
    if (internal_context->supportsWmma)
    {
        ...
        {
            swprintf(pipelineDescription.name, 64, L"FFX_FSR4_MODEL_PASS_%u_POST", passIndex);
    ...
    case fsr4_shaders::Preset::NativeAA:
        memcpy(&internal_context->cbufferData.pass0_weights, weights_native, sizeof(weights_native));
        break;
    case fsr4_shaders::Preset::Quality:
        memcpy(&internal_context->cbufferData.pass0_weights, weights_quality, sizeof(weights_quality));
        break;
    case fsr4_shaders::Preset::Balanced:
        memcpy(&internal_context->cbufferData.pass0_weights, weights_balanced, sizeof(weights_balanced));
        break;
    case fsr4_shaders::Preset::Performance:
        memcpy(&internal_context->cbufferData.pass0_weights, weights_performance, sizeof(weights_performance));
        break;
    case fsr4_shaders::Preset::DRS:
        memcpy(&internal_context->cbufferData.pass0_weights, weights_drs, sizeof(weights_drs));
        break;
    case fsr4_shaders::Preset::UltraPerformance:
        memcpy(&internal_context->cbufferData.pass0_weights, weights_ultraperf, sizeof(weights_ultraperf));
        break;
    }
    return FFX_API_RETURN_OK;
}
...
{
    resourceDescriptions->recurrent = FfxCreateResourceDescription{
    ...
    resourceDescriptions->history = FfxCreateResourceDescription{
        {FFX_HEAP_TYPE_DEFAULT},
    ...
    resourceDescriptions->historyReprojected = FfxCreateResourceDescription{
        {FFX_HEAP_TYPE_DEFAULT},
        { FFX_API_RESOURCE_TYPE_TEXTURE2D, FFX_API_SURFACE_FORMAT_R16G16B16A16_FLOAT, maxUpscaleSize->width, maxUpscaleSize->height, 1, 1, FFX_API_RESOURCE_FLAGS_NONE, (FFX_API_RESOURCE_USAGE_RENDERTARGET | FFX_API_RESOURCE_USAGE_UAV) },
        FFX_API_RESOURCE_STATE_UNORDERED_ACCESS,
    ...
    resourceDescriptions->rcasIntermediary = FfxCreateResourceDescription{
        {FFX_HEAP_TYPE_DEFAULT},
        { FFX_API_RESOURCE_TYPE_TEXTURE2D, FFX_API_SURFACE_FORMAT_R16G16B16A16_FLOAT, maxUpscaleSize->width, maxUpscaleSize->height, 1, 1, FFX_API_RESOURCE_FLAGS_NONE, (FFX_API_RESOURCE_USAGE_RENDERTARGET | FFX_API_RESOURCE_USAGE_UAV) },
        FFX_API_RESOURCE_STATE_UNORDERED_ACCESS,
    ...
    resourceDescriptions->autoExposure = FfxCreateResourceDescription{
        {FFX_HEAP_TYPE_DEFAULT},
    ...
    resourceDescriptions->defaultExposure = FfxCreateResourceDescription{
        {FFX_HEAP_TYPE_DEFAULT},
    ...
    resourceDescriptions->spdAtomicCounter = FfxCreateResourceDescription{
        {FFX_HEAP_TYPE_DEFAULT},
    ...
    resourceDescriptions->lumaMip5 = FfxCreateResourceDescription{
        {FFX_HEAP_TYPE_DEFAULT},
        { FFX_API_RESOURCE_TYPE_TEXTURE2D, FFX_API_SURFACE_FORMAT_R32_FLOAT, RoundUpDiv(maxRenderSize->width, (uint32_t)16), RoundUpDiv(maxRenderSize->height, (uint32_t)16), 1, 1, FFX_API_RESOURCE_FLAGS_NONE, (FFX_API_RESOURCE_USAGE_RENDERTARGET | FFX_API_RESOURCE_USAGE_UAV) },
        FFX_API_RESOURCE_STATE_UNORDERED_ACCESS,
    ...
    resourceDescriptions->scratchBuffer = FfxCreateResourceDescription{
        {FFX_HEAP_TYPE_DEFAULT},
    ...
    // Initializer Buffer
    resourceDescriptions->initializerBuffer = FfxCreateResourceDescription{
        {FFX_HEAP_TYPE_DEFAULT},
    ...
    resourceDescriptions->initializerBufferUpload = FfxCreateResourceDescription{
    ...
    // Debug Information
    resourceDescriptions->debugInformation = FfxCreateResourceDescription{
        {FFX_HEAP_TYPE_DEFAULT},
...
ffxReturnCode_t ffxProvider_FSR4::CreateContext(ffxContext* context, ffxCreateContextDescHeader* header, Allocator& alloc)
{
    ...
    if (auto desc = ffx::DynamicCast<ffxCreateContextDescUpscale>(header))
    {
        ...
        const char* envVarExists = getenv("MLSR-WATERMARK");
        if (envVarExists)
        {
...
ffxReturnCode_t ffxProvider_FSR4::DestroyContext(ffxContext* context, Allocator& alloc)
{
    VERIFY(context, FFX_API_RETURN_ERROR_PARAMETER);
    VERIFY(*context, FFX_API_RETURN_ERROR_PARAMETER);
...
ffxReturnCode_t ffxProvider_FSR4::Query(ffxContext* context, ffxQueryDescHeader* header) const
{
    ...
    case FFX_API_QUERY_DESC_TYPE_UPSCALE_GPU_MEMORY_USAGE_V2:
    {
        ...
        for (size_t i = 0; i < resourceCount; ++i)
        {
            ...
            // TODO: implement ffxGetResourceSizeFromDescriptionVK for accurate memory usage reporting
            // For now, estimate based on resource description dimensions and format
            size = 0;
...
ffxReturnCode_t ffxProvider_FSR4::Dispatch(ffxContext* context, const ffxDispatchDescHeader* header) const
{
    ...
    case FFX_API_DISPATCH_DESC_TYPE_UPSCALE:
    {
        ...
        if (internal_context->supportsWmma)
        {
            dispatchSizes[1] = fsr4_shaders::DispatchSize{RoundUpDiv(std::get<0>(encoderResolutions[0]), 16u), RoundUpDiv(std::get<1>(encoderResolutions[0]), 8u), 1};
            dispatchSizes[2] = dispatchSizes[1];

            dispatchSizes[3] = fsr4_shaders::DispatchSize{RoundUpDiv(std::get<0>(encoderResolutions[1]), 32u), std::get<1>(encoderResolutions[1]), 1};
            dispatchSizes[4] = fsr4_shaders::DispatchSize{RoundUpDiv(std::get<0>(encoderResolutions[1]), 32u), RoundUpDiv(std::get<1>(encoderResolutions[1]), 4u), 1};
            dispatchSizes[5] = dispatchSizes[4];

            dispatchSizes[6] = fsr4_shaders::DispatchSize{RoundUpDiv(std::get<0>(encoderResolutions[2]), 32u), std::get<1>(encoderResolutions[2]), 1};
            dispatchSizes[7] = dispatchSizes[6];
            dispatchSizes[8] = dispatchSizes[6];

            dispatchSizes[9] = dispatchSizes[6];
            dispatchSizes[10] = fsr4_shaders::DispatchSize{RoundUpDiv(std::get<0>(encoderResolutions[1]), 32u), RoundUpDiv(std::get<1>(encoderResolutions[1]), 4u), 1};
            dispatchSizes[11] = fsr4_shaders::DispatchSize{RoundUpDiv(std::get<0>(encoderResolutions[1]), 32u), std::get<1>(encoderResolutions[1]), 1};

            dispatchSizes[12] = fsr4_shaders::DispatchSize{RoundUpDiv(std::get<0>(encoderResolutions[0]), 16u), RoundUpDiv(std::get<1>(encoderResolutions[0]), 8u), 1};

            // Padding
            const fsr4_shaders::DispatchSize outputPadding = {0, 1, 1};
            paddingDispatchSizes[0] = fsr4_shaders::DispatchSize{uint32_t(std::ceil(GetPaddingAmount(encoderResolutions[0], outputPadding) / 32)),1,1};
            paddingDispatchSizes[1] = paddingDispatchSizes[0];
            paddingDispatchSizes[2] = paddingDispatchSizes[0];

            paddingDispatchSizes[3] = fsr4_shaders::DispatchSize{uint32_t(std::ceil(GetPaddingAmount(encoderResolutions[1], outputPadding) / 32)),1,1};
            paddingDispatchSizes[4] = paddingDispatchSizes[3];
            paddingDispatchSizes[5] = paddingDispatchSizes[3];

            paddingDispatchSizes[6] = fsr4_shaders::DispatchSize{uint32_t(std::ceil(GetPaddingAmount(encoderResolutions[2], outputPadding) / 32)),1,1};
            paddingDispatchSizes[7] = paddingDispatchSizes[6];
            paddingDispatchSizes[8] = paddingDispatchSizes[6];

            paddingDispatchSizes[9] = fsr4_shaders::DispatchSize{uint32_t(std::ceil(GetPaddingAmount(encoderResolutions[1], outputPadding) / 32)),1,1};
            paddingDispatchSizes[10] = paddingDispatchSizes[9];

            paddingDispatchSizes[11] = fsr4_shaders::DispatchSize{uint32_t(std::ceil(GetPaddingAmount(encoderResolutions[0], outputPadding) / 32)),1,1};
        ...
        {
            dispatchSizes[1] = fsr4_shaders::DispatchSize{RoundUpDiv(std::get<0>(encoderResolutions[0]), 64u), std::get<1>(encoderResolutions[0]), 1};
            dispatchSizes[2] = dispatchSizes[1];

            dispatchSizes[3] = fsr4_shaders::DispatchSize{RoundUpDiv(std::get<0>(encoderResolutions[1]), 64u), std::get<1>(encoderResolutions[1]), 1};
            dispatchSizes[4] = dispatchSizes[3];
            dispatchSizes[5] = dispatchSizes[3];

            dispatchSizes[6] = fsr4_shaders::DispatchSize{RoundUpDiv(std::get<0>(encoderResolutions[2]), 64u), std::get<1>(encoderResolutions[2]), 1};
            dispatchSizes[7] = dispatchSizes[6];
            dispatchSizes[8] = dispatchSizes[6];

            dispatchSizes[9] = fsr4_shaders::DispatchSize{RoundUpDiv(std::get<0>(encoderResolutions[2]), 8u), RoundUpDiv(std::get<1>(encoderResolutions[2]), 8u), 1};
            dispatchSizes[10] = fsr4_shaders::DispatchSize{RoundUpDiv(std::get<0>(encoderResolutions[1]), 64u), std::get<1>(encoderResolutions[1]), 1};
            dispatchSizes[11] = dispatchSizes[10];

            dispatchSizes[12] = fsr4_shaders::DispatchSize{RoundUpDiv(std::get<0>(encoderResolutions[0]), 64u), std::get<1>(encoderResolutions[0]), 1};
        }
        ...
        if (internal_context->supportsWmma)
        {
            ...
            scheduleDispatch(internal_context, &internal_context->model_pso[0], RoundUpDiv(upscaleWidth, (uint32_t)64), RoundUpDiv(upscaleHeight, (uint32_t)2), 1);
        ...
        if (internal_context->supportsWmma)
        {
            scheduleDispatch(internal_context, &internal_context->model_pso[13], RoundUpDiv(upscaleWidth, (uint32_t)64), RoundUpDiv(upscaleHeight, (uint32_t)2), 1);
        }
        else
        {
        ...
        if (debugViewEnabled)
        {
            scheduleDispatch(internal_context, &internal_context->debug_view_pso, RoundUpDiv(upscaleWidth, (uint32_t)8), RoundUpDiv(upscaleHeight, (uint32_t)8), 1);
        }
        ...
        if (internal_context->watermark)
        {
            ...
            snprintf(watermarkMessage.data(), watermarkMessage.size(), "MLSR Upscale %s\nBuild time: %s %s\nGit branch: %s  commit: %s\nColorspace: %.*s",
                internal_context->supportsWmma ? "wmma" : "v07-i8",
                __DATE__,
                __TIME__,
                GIT_COMMIT_SUBJECT,
                GIT_HASH,
                (int)colorspaceName.size(), colorspaceName.data());
